...
public final class BlockBiggerCreatingTable extends BlockContainer
{
	...
	public boolean onBlockActivated(final World world, final BlockPos blockPos, final IBlockState state, final EntityPlayer entityPlayer, final EnumHand hand, final EnumFacing facing, final float hitX, final float hitY, final float hitZ)
	{
		if (world != null) {
			...
			if (tileEntity instanceof TileEntityBigCreatingTable)
				FMLNetworkHandler.openGui(entityPlayer, BiggerCraftingTables.instance, BiggerCraftingTables.GUI_ID_BIG_CREATING_TABLE, world, blockPos.getX(), blockPos.getY(), blockPos.getZ());
			else if (tileEntity instanceof TileEntityHugeCreatingTable)
				FMLNetworkHandler.openGui(entityPlayer, BiggerCraftingTables.instance, BiggerCraftingTables.GUI_ID_HUGE_CREATING_TABLE, world, blockPos.getX(), blockPos.getY(), blockPos.getZ());
			else if (tileEntity instanceof TileEntityGiantCreatingTable)
				FMLNetworkHandler.openGui(entityPlayer, BiggerCraftingTables.instance, BiggerCraftingTables.GUI_ID_GIANT_CREATING_TABLE, world, blockPos.getX(), blockPos.getY(), blockPos.getZ());
			else
				return false;
		}
		return true;
	...
	public void onBlockPlacedBy(final World world, final BlockPos blockPos, final IBlockState blockState, final EntityLivingBase entityLivingBase, final ItemStack itemStack)
	{
		if (world == null)
			return;
		final TileEntity tileEntity = world.getTileEntity(blockPos);
		if (tileEntity instanceof TileEntityBiggerCreatingTable && itemStack.hasTagCompound())
			((TileEntityBiggerCreatingTable) tileEntity).readCustomNBT(itemStack.getTagCompound());
	}
